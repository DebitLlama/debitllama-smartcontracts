...
import "../Errors.sol";
import "./ExtractableFee.sol";

//   o__ __o__/_  \o       o/   o__ __o     o__ __o__/_   o__ __o        __o__   o          o    o__ __o__/_   o          o   ____o__ __o____          o           o
//  <|    v        v\     /v   <|     v\   <|    v       <|     v\         |    <|\        /|>  <|    v       <|\        <|>   /   \   /   \          <|>         <|>
//  < >             <\   />    / \     <\  < >           / \     <\       / \   / \\o    o// \  < >           / \\o      / \        \o/               / \         / \
//   |                \o/      \o/     o/   |            \o/     o/       \o/   \o/ v\  /v \o/   |            \o/ v\     \o/         |              o/   \o       \o/
//   o__/_             |        |__  _<|/   o__/_         |__  _<|         |     |   <\/>   |    o__/_         |   <\     |         < >            <|__ __|>       |
//   |                / \       |           |             |       \       < >   / \        / \   |            / \    \o  / \         |             /       \      / \
//  <o>             o/   \o    <o>         <o>           <o>       \o      |    \o/        \o/  <o>           \o/     v\ \o/         o           o/         \o    \o/
//   |             /v     v\    |           |             |         v\     o     |          |    |             |       <\ |         <|          /v           v\    |
//  / \  _\o__/_  />       <\  / \         / \  _\o__/_  / \         <\  __|>_  / \        / \  / \  _\o__/_  / \        < \        / \        />             <\  / \ _\o__/_
...
/**
   ...
   DirectDebitWithMEV contains extractable fee that allows searchers to solve intents for rewards
   Using MEV extraction for benefit could help decentralize relaying transactions by relying on searchers who are looking for quick profit.
   This contract has an extra added fee and only allows whitelisted addresses to process direct debit transactions
 */
abstract contract DirectDebitWithMEV is
    ReentrancyGuard,
    DirectDebitErrors,
    DirectDebitEvents,
    Pausable,
    ...
    ExtractableFee
{
   ...
   */
    function calculateFee(
        ...
        public
        view
        returns (uint256 ownerFee, uint256 solverFee, uint256 payment)
    {
        ownerFee = amount.div(ownerFeeDivider);
        solverFee = amount.div(solverFeeDivider);
        payment = amount.sub(ownerFee.add(solverFee));
    }
    ...
    */
    ...
    function _processPaymentIntent(
        ...
        // Calculate the fee
        (uint256 ownerFee, uint256 solverFee, uint256 payment) = calculateFee(
            debit[3]
        );
        ...
        if (address(accounts[hashes[1]].token) != address(0)) {
            ...
            // Send the fee to the solver and check if the solver is authorized
            _processTokenWithdraw(hashes[1], _getSolver(hashes[0]), solverFee);
        } else {
            // Transfer the eth
            ...
            // Send the fee to the solver and check if the solver is authorized
            _processEthWithdraw(hashes[1], _getSolver(hashes[0]), solverFee);
        }
